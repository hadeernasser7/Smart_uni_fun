...
app.eventHub("forwardAttendanceEvent", {
    ...
    handler: async (messages, context) => {
        ...
        for (const event of events) {
            ...
            switch (eventType) {
                case "enrollment":
                    ...
                    if (event.success === false) {
                        context.log(
                            `Enrollment failed on device ${event.deviceId}: ${event.error || "unknown_error"}. Skipping register.`,
                        );
                        continue;
                    }
                ...
                default:
                    context.log(
                        "Skipping event: unsupported 'type' field: ",
                        eventType,
                    );
                    continue;
            }
            ...
            const backendUrl =
                process.env.BACKEND_API_URL.replace(/\/$/, "") + endpoint;

            try {
                context.log(
                    `Forwarding ${eventType} event from ${event.deviceId} to ${backendUrl}`,
                );
                ...
                if (!response.ok) {
                    ...
                    context.log(
                        `Backend error details: Status ${response.status} | Msg: ${errorText}`,
                    );
                    // Throwing forces Event Hub retry loop if endpoint experiences temporary failure (5xx)
                    if (response.status >= 500) {
                        throw new Error(
                            `Transient backend error returned status ${response.status}`,
                        );
                    }
                } else {
                    context.log(
                        `Successfully forwarded ${eventType} telemetry to backend.`,
                    );
                }
            } catch (err) {
                context.log(
                    "Network transmission or forwarding failure:",
                    err.message,
                );
